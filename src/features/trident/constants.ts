...
export const TABLE_WRAPPER_DIV_CLASSNAME = 'overflow-x-auto backdrop-blur-border rounded shadow-md backdrop-blur'
export const TABLE_TABLE_CLASSNAME = 'w-full border-collapse'
// @ts-ignore TYPE NEEDS FIXING
export const TABLE_TR_TH_CLASSNAME = (i, length) =>
  classNames('text-primary text-sm py-3', i === 0 ? 'pl-4 text-left' : 'text-right', i === length - 1 ? 'pr-4' : '')
...
export const TABLE_TBODY_TD_CLASSNAME = (i, length) =>
  classNames('py-3 flex items-center', i === 0 ? 'pl-4 justify-start' : 'justify-end', i === length - 1 ? 'pr-4' : '')
